        ...
        try {
                ...
                if (jsonData.data && jsonData.data.length > 0) {
                        ...
                        jsonData.data.forEach((tour) => {
                                ...
                                tour.places.forEach((place) => {
                                        ...
                                        if (place.famous_locations && place.famous_locations.length > 0) {
                                                countryMap[country].allImages.push(
                        ...
                        Object.values(countryMap).forEach((countryData) => {
                                ...
                                const avgRating =
                                        countryData.ratings.reduce((a, b) => a + b, 0) / countryData.ratings.length;
...
async function fetchDiscountedTours() {
        try {
                ...
                discountedTours = (tourData.tours || []).filter(
...
if (tripForm && tripResults) {
        tripForm.addEventListener("submit", (e) => {
                ...
                const matches = tripData.filter((trip) => {
                        ...
                        const matchRegion =
                                region.toLowerCase() === "all" || trip.region.toLowerCase() === region.toLowerCase();
...
function renderSpecialOffers(page = 1) {
        ...
        // Pagination với dấu "..." - giống blog
        const totalPages = Math.ceil(discountedTours.length / offersPerPage);
        const pageNumbersSpan = offersPagination.querySelector(".page-numbers");
        const prevBtn = offersPagination.querySelector(".prev");
        const nextBtn = offersPagination.querySelector(".next");

        // Hiển thị/ẩn pagination
        offersPagination.style.display = totalPages <= 1 ? "none" : "flex";
        ...
        for (let i = startPage; i <= endPage; i++) {
                ...
                pageNumbersSpan.appendChild(link);
        }
}
...
// Event listeners cho pagination prev/next buttons
const offersPagination = document.getElementById("offersPagination");
if (offersPagination) {
        const prevBtn = offersPagination.querySelector(".prev");
        const nextBtn = offersPagination.querySelector(".next");
        ...
        if (pageNumbersSpan) {
                pageNumbersSpan.addEventListener("click", (e) => {
                        if (e.target.tagName === "A" && e.target.dataset.page) {
                                e.preventDefault();
                                currentOfferPage = parseInt(e.target.dataset.page, 10);
                                renderSpecialOffers(currentOfferPage);
                                // Scroll to offers section
                                const offersSection = document.querySelector(".special-offers");
                                if (offersSection) {
